-- Manual Database Installation Script for FBR POS Integration
...
-- Add FBR fields to items table (if not exists)
ALTER TABLE `tblitems` 
ADD COLUMN `pct_code` varchar(20) DEFAULT NULL;

-- Add FBR fields to invoices table (if not exists)
ALTER TABLE `tblinvoices` 
ADD COLUMN `fbr_invoice_number` varchar(100) DEFAULT NULL,
ADD COLUMN `fbr_status` varchar(20) DEFAULT 'pending',
ADD COLUMN `fbr_qr_code` text DEFAULT NULL;
...
-- Add module options (these will be handled by the module activation normally)
-- INSERT INTO `tbloptions` (`name`, `value`, `autoload`) VALUES
-- ('fbr_pos_enabled', '1', 1),
-- ('fbr_pos_auto_send', '1', 1),
-- ('fbr_pos_tax_rate', '17.00', 1),
-- ('fbr_pos_server_url', 'https://fbr.gov.pk/api/pos', 1);